...
function proto.IsImageLayer(k,v) if(v.filenames)then for i,e in pairs(v.filenames)do if(e:find(".png"))then return true end end end return v.layers or (v.filename and v.filename:find(".png")) end
...
function proto.MultiplyOffsets(v,z) --if(v[1] and istable(v[1]) and not vector.is_zero(v[1]) and not vector.is_zero(v[2]))then
	for a,b in pairs(v)do if(istable(b))then for c,d in pairs(b)do v[a][c]=d*z end else v[a]=b*z end end --else vector.set(v,vector(v)*z) --v[1]=v[1]*z v[2]=v[2]*z
end --end
