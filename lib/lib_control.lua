...
function entity.GetFluidTemperature() local fb=v.fluidbox if(fb and fb[1])then return fb[1].temperature end return 15 end
...
function entity.BalanceFluidPair(a,b)
	...
	if(aff=="steam")then local temp=15 local at=warptorio.GetSteamTemperature(a) local bt=entity.GetFluidTemperature(b) temp=math.max(at,bt)
...
function entity.ShiftFluid(a,b)
	...
	if(aff=="steam")then
		local temp=15 local at=warptorio.GetFluidTemperature(a) local bt=warptorio.GetFluidTemperature(b) temp=math.max(at,bt)
